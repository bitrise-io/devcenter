function initialConfig() {
  ...
  (function(w,d,s,l,i){
    ...
    /* Add fields to GTM dataLayer */
    var dataLayerCookie = document.cookie.split('; ').find(row => row.startsWith('bDL='));
    if (dataLayerCookie) {
      var cookieInfo = decodeURIComponent(dataLayerCookie.split('=')[1]);
      var parsedInfo = JSON.parse(cookieInfo);
      var validEvents = Object.keys(parsedInfo).filter(function(event) {return Boolean(parsedInfo[event])});
      w[l].push(pick(validEvents, parsedInfo));
    }
