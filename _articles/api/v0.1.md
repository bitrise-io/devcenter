...
_Note: the_ `_next_` _property of the_ `_paging_` _object is only included if there's at least one more page available. If there's no_ `_next_` _property inside_ `_paging_` _that means that there's no more page to retrieve._
...
_Note: the_ `_/me/apps_` _endpoint is also working and the same as above, but it's deprecated_
