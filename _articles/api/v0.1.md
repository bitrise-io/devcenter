...
{% include message_box_html type="note" title="My message" content=" The `__/me/apps__` endpoint is also working and the same as above, but it's**eprecate** "%}
...
{% include message_box.html type="note" title="My message" content=" The previous `_/apps/{APP-SLUG}/provisionig-profiles/{PROVISIONING-PROFILE-SLUG}/uploaded_` endpoint will have the same effect as this one with the request body `{\"processed\":true}`.
"%}
