...
So we call `bitrise-init --ci config` in the sample project’s root directory, and in the generated `scan_result.yml` file we replace the step version with `%s` and we use fmt.Sprintf to inject the latest defined step versions into the config.
