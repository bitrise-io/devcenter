...
We will use our `Script` and `Deploy to Bitrise.io` Steps: the `Script` Step executes what you insert in its field and the `Deploy to Bitrise.io` Step delivers your test result to the `APPS & ARTIFACTS` tab of your Build's page.
...
3. Insert the following command to the `Script content` input field.
   ...
   You can find the logs.txt file in the bitrise deploy dir: this logs.txt file contains the outputs of the Appium server. This file would also be downloadable in the Apps and Artifacts tab.
