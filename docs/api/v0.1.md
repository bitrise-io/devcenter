...
_Note: If build is finished the logs will be archived, in this case there are no filtering options, the filter parameters won't have any effect on the content of the response, the whole archived log will be retrieved. Also different in this case the timestamp field, which has ISO 8601 time format._
